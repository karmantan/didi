...
def _occupation_major_group(column: str = "ttsc1_kldb1988") -> pl.Expr:
    # KldB 1988 occupation codes at this level (Berufsordnung) are 3 digits, not 4
    # (Bundesagentur fuer Arbeit: "Ebene der Berufsordnungen (3-Steller)", 334 groups).
    # Padding to width 4 previously meant every real code -- which is at most 3 digits --
    # got an artificial leading zero glued on (e.g. 512 -> "0512"), and slicing character 0
    # then always returned that fake zero instead of the code's real leading digit. Every
    # non-missing person ended up in occ_l1 == "0" regardless of actual occupation, which is
    # why occ_l1_cat collapsed to just {"0", "__MISSING__"} in every diagnostic that used it.
    # Padding to width 3 is the fix: it makes the padding a no-op for genuine 3-digit codes,
    # so the extracted first digit is the code's own leading digit, not filler.
    #
    # This is NOT the same as stripping leading zeros / taking the first non-zero digit --
    # some genuine Berufsordnung codes have a real leading zero (major group 0 covers several
    # 0xx codes), and skipping past a real leading zero would misclassify those into whatever
    # group their second digit implies. Fixed-width padding to the code's true width preserves
    # a real leading zero as a real leading zero; "skip zeros" would silently destroy it.
    #
    # Safety note: this assumes no value in this column exceeds 999. If the underlying source
    # ever supplies a 4-digit KldB code (a different classification level than Berufsordnung),
    # zfill(3) would leave it unpadded at 4 digits and slice(0,1) would take its true first
    # digit anyway -- correct for a 4-digit code, but worth spot-checking after re-running the
    # pipeline that max(ttsc1_kldb1988) stays under 1000, since a systematic switch to a wider
    # code (e.g. Berufsklasse) would need this reconsidered.
    code = pl.col(column).cast(pl.Int32, strict=False)
    padded = pl.when(code.is_not_null()).then(code.cast(pl.Utf8).str.zfill(3)).otherwise(None)
