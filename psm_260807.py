...
longitudinal survival-mediational g-computation outputs. The script performs a total rebuild by default.
...
longitudinal mediation uses annual time-varying histories, interventional effects,
Monte-Carlo g-computation, observation-process weighting, and an indexed matched-set bootstrap; and MSK follow-up includes an observation-gap sensitivity.
"""
...
USER_CONFIG = {
    ...
    # Figures, MSK observation-gap sensitivity, and longitudinal causal mediation.
    "generate_figures": True,
    "run_longitudinal_mediation": True,
    "run_msk_dropout_sensitivity": True,
    "mediation_bootstrap_replicates": 200,
    # The mediation decomposition uses the common five-year horizon available to every
    # 2012-2018 treatment cohort. Treatment is fixed at t0; first MSK entry is an
    # absorbing time-varying mediator. Annual histories from completed prior years are
    # used to preserve temporal ordering.
    "gcomp_horizon_years": 5,
    "gcomp_simulation_repetitions": 25,
    "gcomp_bootstrap_simulation_repetitions": 5,
    "gcomp_use_observation_ipcw": True,
    "gcomp_observation_probability_floor": 0.02,
    "gcomp_ipcw_cap": 20.0,
    # These are post-divorce variables, updated annually. Amounts are modelled on the
    # log(1+x) scale. income_source_status preserves the joint earnings/pension pattern.
...
class PipelineConfig:
    ...
    gcomp_bootstrap_simulation_repetitions: int = 5
...
def _streaming_collect_kwargs() -> dict[str, object]:
    """Return the streaming collect syntax for the installed Polars generation.

    The secure-machine target is Polars 0.20.16, whose documented API is
    ``collect(streaming=True)``.  Newer Polars releases use
    ``collect(engine="streaming")``.  We deliberately choose the candidate order from
    ``pl.__version__`` before probing: Polars 0.20 accepts extra ``**_kwargs`` in
    ``collect``, so probing ``engine="streaming"`` first can appear to succeed even
    though it is not the 0.20 streaming switch.
    """
...
def _can_retry_polars_nonstreaming(exc: BaseException) -> bool:
    """Return True only for execution/API failures for which eager retry is sensible.

    ``pyo3_runtime.PanicException`` inherits directly from BaseException, which is why
    the callers cannot use only ``except Exception``.  We nevertheless re-raise
    interrupts, generator shutdown, memory exhaustion and filesystem failures instead of
    turning them into an eager full-data collect.
    """
...
def _safe_collect(lf: pl.LazyFrame, **extra_kwargs) -> pl.DataFrame:
    ...
    mortality/MSK followup, and longitudinal mediation code. Symptoms range from a spurious
...
def _pandas_arrow_roundtrip_status() -> dict[str, object]:
    """Check the hidden PyArrow dependency used by Polars<->pandas conversions.

    The pipeline deliberately uses pandas/scikit-learn after compacting the data, and
    therefore calls both ``LazyFrame/DataFrame.to_pandas`` and ``pl.from_pandas``.
    On the secure machine this must work before a long rebuild starts.
    """
    try:
...
def sink_parquet(frame: pl.LazyFrame, path: str | Path, row_group_size: int = 200_000) -> Path:
    ...
    try:
        ...
        # The catch is deliberately broader than InvalidOperationError: on 0.20.16 the
        # streaming sink can also corrupt schema resolution and surface as a spurious
        # ColumnNotFoundError/SchemaError (same disease as _safe_collect documents for
        # streaming collect -- reproduced 2026-08-08 on the pre-history checkpoint,
        # where the sink raised ColumnNotFoundError while the non-streaming engine
        # handled the identical plan), or panic at the Rust level
        # (pyo3_runtime.PanicException, a BaseException subclass an ordinary
        # `except Exception` cannot catch). KeyboardInterrupt/SystemExit re-raise above.
        log.warning(
...
def build_analysis_panel(config: PipelineConfig, force: bool = False) -> Path:
    ...
    frame = frame.with_columns([
        ...
        # Moved out of the previous with_columns call: expressions in a single
        # with_columns are evaluated in parallel against the INPUT schema, so an
        # expression cannot reference an alias created in the same call (true on
        # every Polars version; reproduced on 0.20.16 and 1.43.2 alike).
...
def plot_longitudinal_mediation_decomposition(
    ...
    """Plot ATT-standardized interventional direct/indirect mortality effects."""
    keys = [
...
def _gcomp_history_columns(config: PipelineConfig, available: set[str]) -> dict[str, list[str]]:
    """Resolve the deliberately small time-varying post-divorce history set.

    These variables are not ordinary downstream regression controls. They are modelled
    as an evolving history because they may be affected by divorce and may in turn affect
    later first-MSK entry and mortality. The joint earnings/pension state is preserved as
    a categorical variable. Unsupported or unavailable columns fail visibly rather than
...
def build_longitudinal_mediation_data(
    ...
    """Build annual histories for the five-year survival mediational g-formula.

    Treatment is fixed at t0. At follow-up year t, first MSK entry and death are modelled
    using baseline X and the *completed prior-year* history L(t-1). Current-year annual
    economic variables are never used to predict an event in the same calendar year,
    because values such as annual earnings can include months after the event. If first
    MSK entry and death share a calendar year, MSK is ordered first, consistent with the
    RQ1 convention that an observed rehabilitation start proves survival to that start.

    Annual-record gaps are treated as censoring of the mediation history at the first gap.
    The fitting routine can use inverse-probability-of-observation weights to represent
    censored trajectories under conditional independent observation. Death itself remains
    taken from the person-level pension-termination record and therefore does not require
    an annual record in the death year.
    """
    ...
    # Current annual values are renamed explicitly because they update the history for the
    # *next* follow-up year; they are never contemporaneous controls of current MSK/death.
    ...
    # Construct L(t-1). For follow-up year 1, the most recent completed history is t0-1,
    # which is already in the matched baseline. From year 2 onward it is the prior annual
    # post-divorce record. The first-gap censoring rule ensures these values are only used
    # while the longitudinal history remains known.
    ...
    # Event-cell audit is deliberately descriptive. It reveals whether the mediation is
    # likely to be power-limited before any causal decomposition is interpreted.
    people = (
...
def _gcomp_feature_columns(config: PipelineConfig, columns: set[str]) -> FeatureColumns:
    # The longitudinal causal models use the full baseline set from the matched design.
    # SMD-based outcome adjustment is not appropriate for mediator-outcome confounding.
...
def _gcomp_fit_binary(
    ...
    model = _unpenalized_logistic_regression(x, y, sample_weight=weights)
    return {
...
def _gcomp_fit_multiclass(
    ...
    attempts = [
        {"penalty": None, "solver": "lbfgs", "max_iter": 1000, "label": "unpenalized_none_lbfgs"},
        {"penalty": "none", "solver": "lbfgs", "max_iter": 1000, "label": "unpenalized_string_lbfgs"},
        {"penalty": "l2", "C": 1e4, "solver": "lbfgs", "max_iter": 2000, "label": "tiny_ridge_fallback_C1e4"},
    ]
    last_error = None
    fitted = None
    fit_label = None
        ...
        for attempt in attempts:
            ...
            try:
                candidate = LogisticRegression(
                    fit_intercept=True, tol=1e-7, random_state=config.seed, **attempt
                )
...
def _gcomp_simulate_history_update(
    ...
    # Enforce the structural meaning of the joint earnings/pension observation state.
...
def _gcomp_simulate_source_event_times(
    ...
    """Simulate the joint first-MSK history under one source divorce intervention.

    Lin et al.'s survival mediational g-formula first generates the joint mediator
    distribution under the source exposure intervention while conceptually setting prior
    survival to one, then randomises that mediator history when evaluating the target
    exposure world.  In this study the mediator is an absorbing first event.  Its entire
    longitudinal history is therefore represented without loss by one event time: 1..H
    for first MSK entry and H+1 for no entry by the horizon.  Randomising event times,
    rather than independently permuting yearly 0/1 statuses, preserves the structural
    rule that first-MSK status can switch only once from 0 to 1.

    No death draws are made in this source step.  This is deliberate: the source mediator
    distribution is generated under the hypothetical prior-survival intervention needed
    to define a time-varying mediator for a survival outcome.  The mortality model enters
    only when Q(d, d_M) is evaluated below.
    """
    horizon = int(config.gcomp_horizon_years)
    ...
    for follow_year in range(1, horizon + 1):
        ...
        if follow_year < horizon:
            # Generate L(t) under the source treatment and source mediator history for
            # every baseline individual.  Survival is conceptually fixed to one in this
            # source-distribution step, matching the survival mediational g-formula.
...
def _gcomp_survival_risk_with_event_times(
    ...
    """Evaluate Q under target divorce status and an assigned joint MSK history.

    The assigned first-MSK event time is an interventional draw from the source world.
    We then generate the target-world time-varying history year by year and multiply the
    conditional survival probabilities over the common horizon.  We do not simulate a
    factual death and truncate the counterfactual history, because Q is the g-formula
    standardisation over the full intervention history; survival enters through the
...
def _gcomp_estimate_q_values(
    ...
    """Monte Carlo evaluation of Q(d, d_M) for the ATT-standardized decomposition."""
    rng = np.random.default_rng(seed)
    rows: list[dict] = []
    for repetition in range(n_simulations):
        # Step 1: generate the full first-MSK event-time distribution under each source
        # divorce intervention, conceptually under prior survival. Step 2: randomise the
        # joint mediator histories by permuting event times across the ATT baseline
        # population. This follows the mediational g-formula's random-intervention idea
        # while preserving the absorbing first-event structure.
...
def fit_longitudinal_mediation(
    ...
    """Estimate five-year interventional mediation effects and matched-pair bootstrap CIs."""
    ...
    for replicate in range(n_bootstrap):
        ...
        for new_group, old_group in enumerate(sampled, start=1):
            ...
            piece["match_group"] = np.int64(replicate * (len(groups) + 1) + new_group)
            pieces.append(piece)
        ...
        try:
            with warnings.catch_warnings():
                ...
                estimate = _fit_longitudinal_mediation_once(
                    boot,
                    config,
                    n_simulations=config.gcomp_bootstrap_simulation_repetitions,
                    seed=config.seed + 100_000 + replicate,
                )
        ...
        if (replicate + 1) % progress_step == 0 or replicate + 1 == n_bootstrap:
            ...
            log.info(
                ...
                replicate + 1, n_bootstrap, n_success,
            )
...
def run_longitudinal_mediation_only(
    ...
    """Run the longitudinal mediation stage after a completed matched main run."""
    config.validate()
    # This stage deliberately reuses the completed matched run. Do not call
    # initialize_run() here: a standalone mediation invocation may differ only in
    # mediation/reporting settings from the run that created the matched artifacts,
    # and re-initialising on that altered config would create a false signature
    # mismatch. The required upstream files are checked explicitly below.
    config.ensure_dirs()
